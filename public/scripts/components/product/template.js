...
export function createElement(){
    const html = `
        <div class="left-section">
            ...
            <div class="product-images-section">
                <div class="product-image-display js-product-image-display">
                        <img src="${products.product.coverImage}" alt="">
                </div>
        ...
        <div class="right-section">
            <h4 class="product-name">${products.product.name}</h4>
            <p class="product-price">$${formatCurrency(products.product.priceCents)}</p>
            <p class="product-category">Category: <a href="/products?category=${products.product.category._id}">${products.product.category.name}</a></p>
    ...
    function renderProductImages(){
        let html = `
            <div class="product-image js-product-image" data-image="${products.product.coverImage}" data-image-index="0">
                <div class="product-image-selected js-image-select"></div>
                <img src="${products.product.coverImage}" alt="" style="opacity: 1">
            </div>
        `;
        products.product.images.forEach((image, index) => {
            html += `
